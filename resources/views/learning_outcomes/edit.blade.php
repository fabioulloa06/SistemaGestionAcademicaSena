    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="flex justify-start">
                <a href="{{ route('competencias.learning_outcomes.index', $learningOutcome->competencia) }}" class="inline-flex items-center px-4 py-2 bg-gray-200 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest hover:bg-gray-300 focus:bg-gray-300 active:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                <div class="p-4 bg-gradient-to-r from-sena-50 to-sena-100 border-b border-sena-200">
                    <p class="text-sm text-gray-700">
                        <span class="font-semibold">Competencia:</span> {{ $learningOutcome->competencia->codigo }} - {{ $learningOutcome->competencia->nombre }}
                    </p>
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                <div class="p-6 bg-gradient-to-r from-sena-50 to-sena-100 border-b border-sena-200">
                    <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                        ...
                        Editar: {{ $learningOutcome->codigo }}
                    </h3>
                </div>

                <form action="{{ route('learning_outcomes.update', $learningOutcome) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="activo" 
                                ...
                                <option value="1" {{ old('activo', $learningOutcome->activo) ? 'selected' : '' }}>Activo</option>
                                <option value="0" {{ !old('activo', $learningOutcome->activo) ? 'selected' : '' }}>Inactivo</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="descripcion" 
                                  id="descripcion"
                                  rows="4"
                                  class="w-full rounded-md border-gray-300 shadow-sm focus:border-sena-500 focus:ring-sena-500 @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $learningOutcome->descripcion) }}</textarea>
                        @error('descripcion')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
