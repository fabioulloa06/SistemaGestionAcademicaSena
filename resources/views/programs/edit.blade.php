            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                <div class="p-6 bg-gradient-to-r from-sena-50 to-sena-100 border-b border-sena-200">
                    <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                        ...
                        Editar: {{ $program->nombre }}
                    </h3>
                </div>

                <form action="{{ route('programs.update', $program) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" 
                               name="codigo" 
                               id="codigo"
                               value="{{ old('codigo', $program->codigo) }}"
                    ...
                    <div>
                        ...
                        <textarea name="descripcion" 
                                  id="descripcion"
                                  rows="4"
                                  class="w-full rounded-md border-gray-300 shadow-sm focus:border-sena-500 focus:ring-sena-500 @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $program->descripcion) }}</textarea>
                        @error('descripcion')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" 
                                   name="duracion_meses" 
                                   id="duracion_meses"
                                   value="{{ old('duracion_meses', $program->duracion_meses) }}"
                                   min="1"
                        ...
                        <div>
                            ...
                            <select name="nivel" 
                                ...
                                <option value="Técnico" {{ old('nivel', $program->nivel) == 'Técnico' ? 'selected' : '' }}>Técnico</option>
                                <option value="Tecnología" {{ old('nivel', $program->nivel) == 'Tecnología' ? 'selected' : '' }}>Tecnología</option>
                                <option value="Especialización" {{ old('nivel', $program->nivel) == 'Especialización' ? 'selected' : '' }}>Especialización</option>
                                <option value="Curso Corto" {{ old('nivel', $program->nivel) == 'Curso Corto' ? 'selected' : '' }}>Curso Corto</option>
                            </select>
                            @error('nivel')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" 
                               ...
                               {{ old('activo', $program->activo) ? 'checked' : '' }}>
