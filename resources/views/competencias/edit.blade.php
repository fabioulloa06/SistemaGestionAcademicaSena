            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-200">
                <div class="p-6 bg-gradient-to-r from-sena-50 to-sena-100 border-b border-sena-200">
                    <h3 class="text-lg font-semibold text-gray-900 flex items-center">
                        ...
                        Editar: {{ $competencia->codigo }}
                    </h3>
                </div>

                <form action="{{ route('competencias.update', $competencia) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="codigo" class="block text-sm font-medium text-gray-700 mb-2">
                                Código de la Competencia <span class="text-red-500">*</span>
                            </label>
                            <input type="text" 
                                   name="codigo" 
                                   id="codigo"
                        ...
                        <div>
                            ...
                            <select name="activo" 
                                ...
                                <option value="1" {{ old('activo', $competencia->activo) ? 'selected' : '' }}>Activo</option>
                                <option value="0" {{ !old('activo', $competencia->activo) ? 'selected' : '' }}>Inactivo</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="descripcion" 
                                  id="descripcion"
                                  rows="4"
                                  class="w-full rounded-md border-gray-300 shadow-sm focus:border-sena-500 focus:ring-sena-500 @error('descripcion') border-red-500 @enderror">{{ old('descripcion', $competencia->descripcion) }}</textarea>
                        @error('descripcion')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
